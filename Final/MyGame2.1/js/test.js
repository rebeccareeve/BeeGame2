...
function preload() {
    ...
    this.load.spritesheet("jumperLeft", "../assets/jumpingbeekeeperLeft.png", { frameWidth: 32, frameHeight: 53 });
    this.load.spritesheet("bobber", "../assets/bobbingbeekeeper.png", { frameWidth: 32, frameHeight: 53 });
    this.load.spritesheet("hive", "../assets/beehive.png", { frameWidth: 23, frameHeight: 21 });
    this.load.spritesheet("goldenHive", "../assets/goldenbeehive.png", { frameWidth: 23, frameHeight: 21 });
    this.load.spritesheet("bee", "../assets/bee.png", { frameWidth: 25, frameHeight: 23 });
...
function create() {
    ...
    music.overground = this.sound.add('grassLandMusic', { loop: true, volume: 0.5 });

    // player spawn from tiled
    ...
    scoreText = this.add.text(-900, -650, 'Score: 0', { fontSize: '64px', fill: '#111' });
    ...
    this.anims.create({
        key: 'fly',
        frames: this.anims.generateFrameNumbers('bee', { start: 0, end: 10}),
        frameRate: 15,
        repeat: -1
    });
...
function createAnimations() {
    ...
    this.anims.create({
        key: "jumpLeft",
        frames: this.anims.generateFrameNumbers("jumperLeft", { frames: [0, 0] }),
        repeat: -1,
        frameRate: 5
    });
    ...
    this.anims.create({
        key: 'hiveAnims',
        frames: this.anims.generateFrameNumbers('hive', { start: 0, end: 1 }),
        frameRate: 5,
        repeat: -1,
    });
    ...
    this.anims.create({
        key: 'goldenHiveAnims',
        frames: this.anims.generateFrameNumbers('goldenHive', { start: 0, end: 1 }),
        frameRate: 5,
        repeat: -1,
    });
}
...
function showWin() {
    ...
    resultText = this.add.text(14550, 475, 'You Win!', { fontFamily: 'Arial', fontSize: '200px', fill: '#000080' });
    this.physics.pause();
    bees.children.each(function (bee) {
        bee.stopFollow();
        bee.anims.stop();
    }, this)
    //resultText.x = (game.config.width / 2) - (resultText.width / 2);
    //resultText.y = (game.config.height / 2) - (resultText.height / 2);
